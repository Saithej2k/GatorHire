...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Println("Error decoding request body:", err) // Debug log
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(models.ErrorResponse{Error: "Invalid request body"})
		return
	}

	fmt.Println("Received credentials - Email:", credentials.Email, "Password:", credentials.Password) // Debug log
	...
	fmt.Println("User found - Email:", user.Email, "Stored Password:", user.Password) // Debug log

	// Compare passwords
	fmt.Println("Comparing passwords - Stored:", user.Password, "Provided:", credentials.Password) // Debug log
	err = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(credentials.Password))
	if err != nil {
		fmt.Println("Password comparison failed - Error:", err) // Debug log
		w.WriteHeader(http.StatusUnauthorized)
		json.NewEncoder(w).Encode(models.ErrorResponse{Error: "Invalid email or password"})
		return
	}

	fmt.Println("Password comparison successful") // Debug log

	// Generate JWT token
	token, err := utils.GenerateToken(user.ID, user.Email, user.Role)
	if err != nil {
		fmt.Println("Error generating token:", err) // Debug log
